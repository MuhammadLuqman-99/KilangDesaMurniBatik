...
test.describe('Customer Admin @P1', () => {
    test('API-CUS-018: GET /admin/customers — list customers', async ({ customerApi }) => {
        const res = await customerApi.get('/admin/customers');
        expect([200]).toContain(res.status());
    });

    test('API-CUS-020: GET /admin/customers/stats — customer stats', async ({ customerApi }) => {
        const res = await customerApi.get('/admin/customers/stats');
        expect([200]).toContain(res.status());
    });
});
