 ...
 * Service: service-support (port 8010)
 */
...
test.describe('Support - Admin @P1', () => {
    test('API-SUP-008: GET /admin/support/stats — dashboard stats', async ({ supportApi }) => {
        const res = await supportApi.get('/admin/support/stats');
        expect([200]).toContain(res.status());
    });

    test('API-SUP-009: GET /admin/support/tickets — list all tickets', async ({ supportApi }) => {
        const res = await supportApi.get('/admin/support/tickets');
        expect([200]).toContain(res.status());
    });

    test('API-SUP-012: POST /admin/support/tickets/:id/close — close ticket', async ({ supportApi }) => {
        // Get a ticket to close
        const listRes = await supportApi.get('/admin/support/tickets?limit=1');
        if (listRes.status() === 200) {
            ...
            if (tickets.length > 0) {
                ...
                const res = await supportApi.post(`/admin/support/tickets/${ticketId}/close`);
                expect([200, 204, 400]).toContain(res.status()); // 400 if already closed
            }
        }
    });
});
